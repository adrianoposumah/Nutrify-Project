...
export class AdminModel {
  ...
  async getUserById(userId: string): Promise<UserListResponse> {
    try {
      const response = await apiClient.get<ApiResponse<UserListResponse>>(`/users/${userId}`);
      return response.data;
    } catch (error) {
      throw error;
    }
  ...
  async changeUserRole(userIdToChange: string, roleData: ChangeRoleRequest): Promise<AdminMessageResponse> {
    try {
      const response = await apiClient.patch<ApiResponse<AdminMessageResponse>>(`/users/${userIdToChange}/role`, roleData);
      return response.data;
    } catch (error) {
      throw error;
    }
  ...
  async deleteUser(userIdToDelete: string): Promise<AdminMessageResponse> {
    try {
      const response = await apiClient.delete<ApiResponse<AdminMessageResponse>>(`/users/${userIdToDelete}`);
      return response.data;
    } catch (error) {
      throw error;
    }
