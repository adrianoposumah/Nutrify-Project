...
export default function CategoryPage() {
  ...
  const categories = useMemo(() => {
    const uniqueCategories = [...new Set(foodData.food.map((item) => item.category))];
    return ['all', ...uniqueCategories];
  }, []);

  const nations = useMemo(() => {
    const uniqueNations = [...new Set(foodData.food.map((item) => item.nation))];
  ...
  const filteredFoods = useMemo(() => {
    return foodData.food.filter((item) => {
      ...
      const matchesSearch = item.name.toLowerCase().includes(searchTerm.toLowerCase()) || item.nation.toLowerCase().includes(searchTerm.toLowerCase()) || item.origin?.toLowerCase().includes(searchTerm.toLowerCase());

      return matchesCategory && matchesNation && matchesSearch;
    });
  }, [selectedCategory, selectedNation, searchTerm]);
    ...
    <div className="container mx-auto px-4 py-10 lg:py-20">
      <main className="mt-5 ">
        ...
        <div className="text-center space-y-4 py-8">
          ...
          <h1 className="text-4xl lg:text-5xl font-bold bg-gradient-to-r from-orange-600 to-red-600 bg-clip-text text-transparent">Kategori</h1>
        </div>
        ...
        {filteredFoods.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredFoods.map((food) => (
              <Card key={food.id} className="group overflow-hidden border-0 shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2">
                {/* Food Image */}
